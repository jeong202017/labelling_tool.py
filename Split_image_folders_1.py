...
def split_images_into_folders(source_folder, dest_folder, split_ratios, folder_names=('train', 'val', 'test')):
    """
    source_folder 안의 파일들을 split_ratios에 따라 folder_names의 하위 폴더로 나누는 함수
    ...
    for file in train_files:
        shutil.move(os.path.join(source_folder, file), os.path.join(dest_folder, folder_names[0], file))
    for file in val_files:
        shutil.move(os.path.join(source_folder, file), os.path.join(dest_folder, folder_names[1], file))
    for file in test_files:
        shutil.move(os.path.join(source_folder, file), os.path.join(dest_folder, folder_names[2], file))
    
    print("파일 분할 완료!")

# 사용 예제
source_folder = "/home/scps/Desktop/auto_label/jpgs/new/unripe/yeolgwa(crack)"  # 원본 이미지 폴더 경로 입력
dest_folder = "/home/scps/Desktop/jcw_ws/apple"  # 분할된 데이터가 저장될 폴더
